...
export function WorkerAvailabilityEditor({
  hours,
  loading,
  onSave,
  saving,
  workerName,
}: WorkerAvailabilityEditorProps) {
  const [dayHours, setDayHours] = useState<WorkingHourDay[]>([]);
  const [errors, setErrors] = useState<Record<number, string>>({});
  const [copiedFromShop, setCopiedFromShop] = useState(false);
  ...
  const handleCopyFromShopHours = (shopHours: WorkingHourDay[]) => {
    setDayHours(shopHours.map((h) => ({ ...h })));
    setCopiedFromShop(true);
  };
    ...
    >
      <View className="flex-row items-center justify-between">
        <View className="flex-1">
          <AppText variant="heading" className="text-lg">{workerName} — Hours</AppText>
        </View>
      ...
      <AppText variant="caption" className="text-muted-foreground">
        If no custom hours are set, worker follows shop working hours.
      </AppText>
      ...
      <View className="gap-3">
        {!copiedFromShop && (
          <PrimaryButton
            onPress={() => {
              // Pass a callback that parent can provide shop hours
              handleCopyFromShopHours(dayHours.map((h) => ({ ...h, isActive: true })));
            }}
            className="w-full"
          >
